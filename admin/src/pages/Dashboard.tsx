...
import { useGetRegistrationTrendQuery, useGetStatsQuery, useGetUsersMetricsQuery } from '../services/api/dashboardApi';
...
const Dashboard: React.FC = () => {
  ...
  const {isLoading:regLoading,error:regError,data:registrationTrend} = useGetRegistrationTrendQuery();
  const {isLoading:engLoading,error:engError,data:engagementMetrics} = useGetUsersMetricsQuery();

  // Handle errors from API calls
  React.useEffect(() => {
    if (error) {
      const errorMessage = (error as any)?.data?.message || 'Failed to load dashboard statistics';
  ...
  React.useEffect(() => {
    if (regError) {
      const errorMessage = (regError as any)?.data?.message || 'Failed to load registration trends';
      showToast(errorMessage, 'error');
    }
  }, [regError, showToast]);
  ...
  React.useEffect(() => {
    if (engError) {
      const errorMessage = (engError as any)?.data?.message || 'Failed to load engagement metrics';
      showToast(errorMessage, 'error');
    }
  }, [engError, showToast]);
        ...
        <Card className="bg-gradient-to-r from-green-500 to-green-600 text-white">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-3xl font-bold">{dashboardStats?.subscribedUsers || 0}</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="bg-gradient-to-r from-purple-500 to-purple-600 text-white">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-3xl font-bold">{dashboardStats?.totalGames || 0}</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card title="Recent Activity">
        <div className="space-y-4">
          ...
          <div className="flex items-center p-3 bg-gray-50 rounded-lg">
            ...
            <div>
              <p className="text-sm text-gray-600">User subscription activated</p>
          ...
          <div className="flex items-center p-3 bg-gray-50 rounded-lg">
            ...
            <div>
              <p className="text-sm text-gray-600">Daily active users increased by 12%</p>
