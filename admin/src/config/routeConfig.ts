...
export const getAccessibleRoutes = (userPermissions: string[], userRole?: string): RouteConfig[] => {
...
export const canAccessRoute = (routePath: string, userPermissions: string[], userRole?: string): boolean => {
...
export const getRoutesByCategory = (userPermissions?: string[], userRole?: string): Record<string, RouteConfig[]> => {
  // If user is ADMIN, show all routes
  const routesToShow = userRole === 'ADMIN' ? routeConfigs : 
    (userPermissions ? getAccessibleRoutes(userPermissions, userRole) : routeConfigs);
