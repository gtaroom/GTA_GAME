...
function Command({
    ...
    return (
        <CommandPrimitive
            data-slot='command'
            className={cn(
                'text-popover-foreground flex size-full flex-col overflow-hidden rounded-md bg-neutral-800',
                className
            )}
            {...props}
        />
...
function CommandInput({
    ...
    return (
        <div
            ...
            cmdk-input-wrapper=''
        >
            <SearchIcon size={20} className='me-3 text-white/60' />
            <CommandPrimitive.Input
                data-slot='command-input-wrapper'
                className={cn(
                    'flex h-10 w-full rounded-md bg-transparent py-3 text-sm outline-hidden placeholder:text-white/60 disabled:cursor-not-allowed disabled:opacity-50',
                    className
                )}
                {...props}
            />
