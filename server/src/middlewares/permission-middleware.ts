    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (
        ...
        customRole.permissions[
          permission as keyof typeof customRole.permissions
        ]
      ) {
        return next();
      }
 ...
 */
export const requireAdminAccess = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { role } = getUserFromRequest(req);
    ...
    if (AvailableRoles.includes(role as any)) {
      ...
      if (
        permissions &&
        "canAccessEverything" in permissions &&
        permissions.canAccessEverything
      ) {
        return next();
      }
    }
 ...
 */
export const canManageRoles = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
 ...
 */
export const canViewUsers = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { role } = getUserFromRequest(req);
    ...
    if (AvailableRoles.includes(role as any)) {
      ...
      if (
        ...
        (("canViewAllUsers" in permissions && permissions.canViewAllUsers) ||
          ("canViewUserProfiles" in permissions &&
            permissions.canViewUserProfiles))
      ) {
        return next();
      }
    }
    ...
    if (
      customRole &&
      (customRole.permissions.canViewAllUsers ||
        customRole.permissions.canViewUserProfiles)
    ) {
      return next();
    }
 ...
 */
export const canEditUsers = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { role } = getUserFromRequest(req);
 ...
 */
export const canDeleteUsers = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { role } = getUserFromRequest(req);
 ...
 */
export const canViewAnalytics = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { role } = getUserFromRequest(req);
    ...
    if (AvailableRoles.includes(role as any)) {
      ...
      if (
        permissions &&
        "canViewAnalytics" in permissions &&
...
export const canViewSupportTickets = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { role } = getUserFromRequest(req);
    ...
    if (AvailableRoles.includes(role as any)) {
      ...
      if (
        permissions &&
        "canViewSupportTickets" in permissions &&
        permissions.canViewSupportTickets
      ) {
        return next();
      }
    }
...
export const canResolveSupportTickets = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { role } = getUserFromRequest(req);
    ...
    if (AvailableRoles.includes(role as any)) {
      ...
      if (
        permissions &&
        "canResolveSupportTickets" in permissions &&
        permissions.canResolveSupportTickets
      ) {
        return next();
      }
    }
    ...
    throw new ApiError(
      403,
      "You don't have permission to resolve support tickets"
    );
  }
);

export const canManageGames = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { role } = getUserFromRequest(req);
    ...
    throw new ApiError(
      403,
      "You don't have permission to manage games account requests"
    );
  }
);

export const canManageCoupons = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { role } = getUserFromRequest(req);
    ...
    if (AvailableRoles.includes(role as any)) {
      ...
      if (
        permissions &&
        "canManageCoupons" in permissions &&
...
export const canViewAllTransactions = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { role } = getUserFromRequest(req);
    ...
    if (AvailableRoles.includes(role as any)) {
      ...
      if (
        permissions &&
        "canViewAllTransactions" in permissions &&
...
/**
 * Check if user can manage home banners
 */
export const canManageBanners = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const { role } = getUserFromRequest(req);
  ...
  // 2. Check hardcoded/built-in roles (from constants)
  if (AvailableRoles.includes(role as any)) {
    const permissions = rolePermissions[role as keyof typeof rolePermissions];
    if (permissions && 'canManageBanners' in permissions && (permissions as any).canManageBanners) {
      return next();
    }
  }

  // 3. Check database-defined custom roles (This is where your DESIGNER role lives)
  const customRole = await RoleModel.findOne({ name: role, isActive: true });
  if (customRole && customRole.permissions.canManageBanners) {
    return next();
  }

  throw new ApiError(403, "You don't have permission to manage banners");
});
